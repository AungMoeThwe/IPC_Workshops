...
int main(void)
{
	...
	do
	{
		...
		case 1:
			...
			years = (int)totalMonths / 12; //cast the double into int so that we can get years in integers
			months = ((int)totalMonths - (years * 12) + 1);
			printf("\n====================================================\n");
			printf("Filter:   All items\n"); // [option-1]
			printf("Amount:   $%.2lf", totalCost);
			...
			} // if there is a y itl print this note.
			printf("====================================================\n\n");
			break;
		...
		case 2:
			...
			do
			{
				...
				scanf("%d", &priorityNum);
			} while (!(priorityNum >= 1 && priorityNum <= 3)); // loop until correcct priority

			for ( i = 0; i < item; i++)
			{
				if (priority[i] == priorityNum)
				{
			...
			for (i = 0; i < item; i++)
			{
				if (finance[i] == 'y' && priority[i] == priorityNum)
				{
					note = 1;
				}
			}
